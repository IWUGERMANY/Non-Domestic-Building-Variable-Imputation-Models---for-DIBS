# BE_qh3_3_imputation.R
# Julian Bischof
# 26.07.2021
# Abhänigkeiten: BE_imputation.R

#-----------------------------------------
# qh3_3: überwiegende Funktionen der RLTs Kühlen
#-----------------------------------------
...
DB_BE$qh3_3[DB_BE$qh3_3==-8 & DB_BE$qh1==3] <- -7
...
DB_BE_clean <- subset(DB_BE, qh3_3!=-7 & qh3_3!=-8) # wenn qh3_3 = 3 ist überhaut nur eine zentrale Belüftung vorhanden
####

qh3_3 <- DB_BE_clean$qh3_3
...
DB_BE_unclean_qh3_3 <- subset(DB_BE, hk_geb>=0 & bak_grob>=0 & qh3_3==-7)


    # Counting number of imputations in this variable and marking them for later analysis
    qh3_3_imputated <- DB_BE$qh3_3 == -7
    table <- table(qh3_3_imputated)
    qh3_3_n_imputated <- table[2]
    
    qh3_3_imputated[qh3_3_imputated == TRUE] <- 1
...
DB_BE_clean$qh3_3 <- relevel(as.factor(DB_BE_clean$qh3_3), ref = "1")


#----------------------------------------
# 2.1 Regressionsmodel bak_clean und hk_geb_clean zu qh3_3_clean

# Reg <- glm(qh3_3 ~ as.factor(hk_geb) * as.factor(bak), data = DB_BE_clean, family=binomial) # Algorithmus konvergierte nicht  # NAs
# Reg <- glm(qh3_3 ~ as.factor(hk_geb) * as.factor(bak_grob), data = DB_BE_clean, family=binomial) # AIC: 1166 #NAs
#Reg <- glm(qh3_3 ~ as.factor(hk_geb) * as.factor(bak_grob), data = DB_BE_clean, family=binomial, weights = (HRF/Sum_HRF_BE_clean)*N_Clean) # AIC: 1166 #NAs
Reg <- glm(qh3_3 ~ as.factor(hk_geb) * as.factor(bak_grob), data = DB_BE_clean, family=quasibinomial, weights = (HRF/Sum_HRF_BE_clean)*N_Clean) # AIC: #NAs
...
sink("D:/OneDrive/OneDrive - Technological University Dublin/GitHub/DataNWG---BRE-Imputation-for-DIBS/Reg_equations/qh3_3.txt")
...
saveRDS(Reg, "D:/OneDrive/OneDrive - Technological University Dublin/GitHub/DataNWG---BRE-Imputation-for-DIBS/Reg_equations/qh3_3.rds")
...
#DB_BE$prob_qh3_3_1 <- predict(Reg, newdata=DB_BE, type="response") 
prob_qh3_3_1 <- predict(Reg, newdata=DB_BE, type="response") 

# THIRD get subset of missing qh3_3 (or do above with unclean data set and use here onwards)
#####################################################
#Generieren von Un-Clean Subset d.h. das Subset in welchem XXX = -7
DB_BE_unclean_qh3_3 <- subset(DB_BE, hk_geb>=0 & bak_grob>=0 & qh3_3==-7)
...
levels_qh3_3 <- levels(DB_BE_clean$qh3_3)
...
for(i in 1:rows_DB_BE){
  if(DB_BE$qh3_3[i]==-7){
    DB_BE$qh3_3[i] <- sample(levels_qh3_3, size = 1, replace = TRUE, prob = c(prob_qh3_3_1[i],1-prob_qh3_3_1[i]))
    #DB_BE$qh3_3[i] <- sample(levels_qh3_3, size = 1, replace = TRUE, prob = c(DB_BE$prob_qh3_3_1[i],1-DB_BE$prob_qh3_3_1[i]))
  }
}
...
qh3_3 <- DB_BE$qh3_3
Imputierte_Variable <- as.data.frame(cbind(scr_gebaeude_id, qh3_3, qh3_3_imputated)) # mit Geb.ID verbinden, damit klar zuweisbar

# write it as a csv file
write.csv(Imputierte_Variable, 'DB_BE_qh3_3_imputiert.csv')
